...
public class MyPolygonal_old extends MyDrawing {
    private int size;       // 中心から各頂点までの長さ
    private int n;          // n角形 であることを表す
    private double[] xPoints, yPoints;  // 点を格納する配列, double型なのでint型にキャストして使うこと.

    public MyPolygonal_old(int xpt, int ypt, int size, int n, Color lineColor, Color fillColor, int lineWidth) {
        this(xpt, ypt, size, n);
        setLineColor(lineColor);
        setFillColor(fillColor);
        setLineWidth(lineWidth);
    }
    public MyPolygonal_old(int xpt, int ypt, int size, int n) {
        super(xpt, ypt);
        setSize(size);
    ...
    public MyPolygonal_old(int xpt, int ypt, int wpt, int hpt, int n) {
        this(xpt, ypt, wpt, n);
    }
